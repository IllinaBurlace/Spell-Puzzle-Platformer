...
func simplify(input: PackedVector2Array) -> PackedVector2Array:
	...
	curve.add_point(input[input.size() - 1])
	
	var highest: Dictionary = {"id": 0, "dist": 0}
	var current: int = 0
	for point in input:
	...
	if highest.dist > epsilon:
		...
		ret.append_array(simplify(input.slice(highest.id, input.size())))
	else:
		ret = curve.get_baked_points()
